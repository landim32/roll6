...
- [X] T003 Configurar `down_space` como coluna anulável **sem** `HasDefaultValue`/`HasSentinel` (research R1) em `Infra/Context/SimpleTabletopMapContext.cs`
- [X] T004 Gerar a migração `MakeTokenDownSpaceOptional` em `Infra/Migrations/` (`dotnet ef migrations add MakeTokenDownSpaceOptional --project SimpleTabletopMap.Infra --startup-project SimpleTabletopMap.API`, em `backend/`) e conferir que o `AlterColumn` só torna `down_space` anulável e remove o default, sem apagar dados
